...
class Blockchain:
    ...
    def is_chain_valid(self, chain: list) -> bool:
        ...
        while block_index < len(chain):
            ...
            proof = block['proof']
            hash_operation: bytes = hashlib.sha256(str(proof**2 - previous_proof**2).encode()).hexdigest()
            if hash_operation[:4] != '0000':
                return False
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Sunandbean', amount = 10)
...
def connect_node():
    ...
    response = {'message': 'All the nodes are now connected. The Suncoin Blockchain now contains the following nodes',
